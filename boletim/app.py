...
def editar_disciplina(cod):
    ...
    return render_template('nova_disciplina.html', disc=disciplina, editando=True, cod=cod)
...
def salvar_disciplina():
    ...
    if nota1: nota1 = float(nota1.replace(",","."))
    ...
    if editando == "True":
        conexao.execute(
            'UPDATE Disciplina SET nome = ?, nota1 = ?, nota2 = ?, nota3 = ?, nota4 = ?, situacao = ? where cod = ?;',
            [nome, nota1, nota2, nota3, nota4, situacao, cod])
    else:
        conexao.execute('Insert INTO Disciplina (nome, nota1, nota2, nota3, nota4, situacao) values (?,?,?,?,?,?)',
                        [nome, nota1, nota2, nota3, nota4, situacao])
    conexao.commit()
...
## REMOVER
@app.route('/RemoverDisciplina/<int:cod>')
def remover_disciplina(cod):
    conexao = obter_conexao()
    conexao.execute('DELETE From Disciplina where cod = ? ', [cod])
    conexao.commit()
    return redirect('/')
